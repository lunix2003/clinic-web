...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1>{{ trans('global.create') }} {{ trans('menu.contacts') }}</h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">Home</a></li>
          <li class="breadcrumb-item active">{{ trans('global.create') }} {{ trans('menu.contacts') }}</li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
...
<form action="{{route('contact.update',$contact->id)}}" method="post" enctype="multipart/form-data">
  ...
  <div class="row">
    <div class="col-md-8">
      <div class="card card-primary card-outline">
        <div class="card-header">
          <h4 class="float-left">{{ trans('global.create') }} {{ trans('menu.contacts') }}</h4>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
          <input name="name" class="form-control form-control-lg" type="text" placeholder="Name" value="{{$contact->name}}">
          <br>
          <input name="email" class="form-control form-control-lg" type="email" placeholder="Email" value="{{$contact->email}}">
            <br>
            <input name="subject" class="form-control form-control-lg" type="text" placeholder="Subject" value="{{$contact->subject}}">
            <br>
            <div class="form-group">
                <label for="detail">Message</label>
                <div class="input-group">
                  <textarea style="width: 100%;" id="summernote" name="message"
                    class="form-control form-control-lg">{{$contact->message}}</textarea>
                </div>
              </div>
        </div>
        <!-- /.card-body -->
    ...
    <div class="col-md-4">
      <div class="card card-primary card-outline">
        <div class="card-header">
          <h4 class="float-left">{{ trans('global.action') }} {{ trans('menu.contacts') }}</h4>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
          <div class="form-group">
